...
import { CHANGER_GATE_MODES_REQUIRED } from '../config/index.js'
import type { CachedLevel, VerifiedLevel } from '../types/index.js'
...
export const sendJudgeMessage = async ({
	...
	const blockCount = `${emoji(!isOverBlockLimit)} ${level.blocks}${previousBlockCount}`

	const previousCheckpointCount = previousLevel ? ` (${previousLevel.checkpoints})` : ''
	...
	if (previousLevel) {
		embed.setDescription('Level information from before the update is in brackets')
	}
	...
	// merge level.levelAuthors.name and level.levelAuthors.collaborators into one string
	const authors = level.levelAuthors.collaborators
		? `${level.levelAuthors.name} + ${level.levelAuthors.collaborators}`
		: level.levelAuthors.name
	...
	buttons.addComponents([
		new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel('Subscribe to level').setURL(url),
	])
